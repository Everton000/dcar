...
<script type="text/javascript">
    ...
    var handleBarChart = function () {
        ...
        {
            $.post('index.php?app_modulo=home&app_comando=listar_grafico_total_contas_recebidas_mensal',
                function (response)
                {
                    var cont;
                    var data = [];
                    var ticks = [];
                    for (cont = 0; cont < response['mes_atual']; cont++)
                    {
                        data.push([cont, parseInt(response[cont]['dados'])]);
                        ticks.push([cont, response[cont]['mes']]);
                    }

                    $.plot('#bar-chart', [{ label: 'Contas Recebidas', data: data, color: COLOR_BLACK_LIGHTER }], {
                        series: {
                            bars: {
                                show: true,
                                barWidth: 0.6,
    ...
    var handleInteractivePieChart = function () {
        ...
        if ($('#interactive-pie-chart').length !== 0)
        {
            ...
            var cor = '';
            $.post('index.php?app_modulo=home&app_comando=listar_grafico_total_contas',
                function (response)
                {
                    for( var i = 0; i<series; i++)
                    {
                        switch (response[i]['color'])
                        {
                            case 'COLOR_GREY':
                                cor = COLOR_GREY;
                                break;
                            case 'COLOR_BLACK_LIGHTER':
                                cor = COLOR_BLACK_LIGHTER;
                                break;
                            case 'COLOR_ORANGE':
                                cor = COLOR_ORANGE;
                                break;
                        }
                        data[i] = { label: response[i]['label'], data: response[i]['data'], color: cor};
                    }
